...
{$ifdef undo}
    ...
    procedure compacting;
    procedure cmOptUndo( var Msg : tMessage );
      virtual CM_FIRST + cm_OptUndo;
{$endif}
    procedure changeAmDIN(b : boolean);
    procedure cmOptEleAm( var Msg : tMessage );
      virtual CM_FIRST + cm_OptEleAm;
    procedure cmOptEleDIN( var Msg : tMessage );
      virtual CM_FIRST + cm_OptEleDIN;
  end;
...
begin
  WinHelp(hMainWin, HelpFileName, HELP_CONTEXT, 110);
end;
...
begin
  WinHelp(hMainWin, HelpFileName, HELP_CONTEXT, 400);
end;
...
{$endif}
  ...
  WinHelp(hMainWin, HelpFileName, HELP_QUIT, 0);
end;
...
begin
  ...
  do begin
    ...
    then begin
      GetCurDir(@String2, 0);
      StrCat(@String2, filen);
      EleFiles^.Insert(New(pEleFile, Init(alias, @String2)));
    end
    else
  ...
  cmMessageOn(Msg);
  GetPrivateProfileString('GENERAL', 'Collision', 'off', @s, 20, IniFile);
...
{$ifdef test}
  ...
  then begin
    ...
    GetPrivateProfileString(
      'ISUD', 'qnk', 'j',
      @String2, StringLen, IniFile );
    if (StrComp(@String2, 'XDDL3801110000813863')=0)
    then begin
      { erster Start. }
      expired := false;
      startdate := Date;
      ...
      { Start an Tag x. }
      sy := GetPrivateProfileInt('ISUD', 'fhj', 0, IniFile);
      sy := sy xor 2678;
      sy := sy - 3856;
      sm := GetPrivateProfileInt('ISUD', 'oiu', 0, IniFile);
      sm := sm xor 2834;
      sm := sm - 7889;
      sd := GetPrivateProfileInt('ISUD', 'lki', 0, IniFile);
      sd := sd xor 9826;
      sd := sd - 2785;
      if (sm>=1) and (sm<=12) and (sd>=1) and (sd<=31)
      then begin
        startdate := EncodeDate(sy, sm, sd);
        days := Date;
        days := Date-startdate;
        days := 30-days;
        daystogo := round(days);
        if (daystogo<-5) or (daystogo>40)
        then begin
          expired := true;
          daystogo := -daystogo;
        end
...
{$endif}
begin
  Randomize; { Wird eigentlich nicht mehr benötigt. Nur sicherheitshalber. }
  inherited SetupWindow;
  ...
  CurIniFile := StrNew(@String1);
  LoadIni(CurIniFile);
  ChangeIniFile(CurIniFile);
  GetCurDir(@String2, 0);
  StrCat(StrCat(StrCat(@String2, '\setup_'), Language), '.ini');
  SetupIniFile := StrNew(@String2);
  GetPrivateProfileString(
    'GZUI', 'plpsd', '0.0', @String2, StringLen, SetupIniFile);
  if StrComp(@String2, LoKon_Version)>0
  then begin
    LoadIni(SetupIniFile);
    { English ini abschalten. }
    GetCurDir(@String2, 0);
    StrCat(@String2, '\setup_en.ini');
    WritePrivateProfileString('GZUI', 'plpsd', nil, @String2);
    WritePrivateProfileString('ISUD', 'qnk', nil, @String2);
    { Deutsche ini abschalten. }
    GetCurDir(@String2, 0);
    StrCat(@String2, '\setup_de.ini');
    WritePrivateProfileString('GZUI', 'plpsd', nil, @String2);
    WritePrivateProfileString('ISUD', 'qnk', nil, @String2);
    { Auf normale ini Datei umschalten. }
    ChangeIniFile(CurIniFile);
    SaveIni;
  end;
  StrDispose(SetupIniFile);
  StrDispose(CurIniFile);
...
begin
  GetClassName:='LoKon V2.4 MainWin';
end;
...
begin
  ...
  do begin
    ...
    if expired
    then begin
      if (wParam>=cm_ExpiredFirst) and (wParam<=cm_ExpiredLast)
      then begin
        b := false;
        SendMessage(hWindow, wm_Command, cm_About, 0);
      end;
...
{ expire information }
  ...
  then begin
    ...
    y := y + 3856;
    y := y xor 2678;
    wvsprintf(@String2, '%u', y);
    WritePrivateProfileString('ISUD', 'fhj', @String2, IniFile);
    m := m + 7889;
    m := m xor 2834;
    wvsprintf(@String2, '%u', m);
    WritePrivateProfileString('ISUD', 'oiu', @String2, IniFile);
    d := d + 2785;
    d := d xor 9826;
    wvsprintf(@String2, '%u', d);
    WritePrivateProfileString('ISUD', 'lki', @String2, IniFile);
  end
  else begin
    WritePrivateProfileString('ISUD', 'fhj', nil, IniFile);
    WritePrivateProfileString('ISUD', 'oiu', nil, IniFile);
    WritePrivateProfileString('ISUD', 'lki', nil, IniFile);
  end;
{$endif}
end;
...
{$ifdef test}
  ...
  then begin
    if expired
    then begin
      wvsprintf(@String2, 'expired for %i days', daystogo);
      SetDlgItemText(hWindow, 100, @String2);
    end
    else begin
      wvsprintf(@String2, 'expires in %i days', daystogo);
      SetDlgItemText(hWindow, 100, @String2);
    end;
  end
  else begin
...
begin
  WinHelp(hWindow, HelpFileName, HELP_CONTEXT, 103);
end;
...
begin
  ...
  WinHelp(hWindow, HelpFileName, HELP_CONTENTS, 0);
  SendMessage(hWindow, ms_UpdateInfo, 0, 0);
end;
...
begin
  ...
  else WinHelp(hWindow, HelpFileName, HELP_CONTEXT, 103);
  SendMessage(hWindow, ms_UpdateInfo, 0, 0);
end;
...
begin
  SendMessage(hWindow, ms_UpdateInfo, 1032, 0);
  WinHelp(hWindow, HelpFileName, HELP_HELPONHELP, cs_Info);
  SendMessage(hWindow, ms_UpdateInfo, 0, 0);
end;
...
begin
  SendMessage(hWindow, ms_UpdateInfo, 1032, 0);
  WinHelp(hWindow, HelpFileName, HELP_CONTEXT, 900);
  SendMessage(hWindow, ms_UpdateInfo, 0, 0);
end;
...
begin
  ...
  with code
  do begin
    x := GetDlgItemInt(hWindow, 102, nil, false);
    y := GetDlgItemInt(hWindow, 103, nil, false);
...
begin
  MessageBox(hWindow, 'Compacting', nil, mb_ok);
  compacting;
end;

procedure tMainWindow.compacting;
...
procedure tMainWindow.changeAmDIN(b : boolean);
procedure DoIt(p : pEleFile); far;
begin
  if StrComp(p^.Name, 'Elements')=0
  then begin
    String2[0] := #0;
    StrCat(StrCat(@String2, '\ele_'), language);
    if b
    then StrCat(@String2, '\ele_am.ele')
    else StrCat(@String2, '\ele_din.ele');
    p^.ChangeFile(@String2);
  end;
end;
begin
  SendMessage(hWindow, WM_COMMAND, 24338, 0);
  EleFiles^.ForEach(@DoIt);
  SaveIni;
  EleFiles^.FreeAll;
  LoadIni(StrNew(IniFile));
end;

procedure tMainWindow.cmOptEleAm( var Msg : tMessage );
begin
  changeAmDIN(true);
end;

procedure tMainWindow.cmOptEleDIN( var Msg : tMessage );
begin
  changeAmDIN(false);
end;
