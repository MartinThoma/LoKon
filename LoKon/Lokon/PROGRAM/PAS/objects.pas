...
{ TStreamRec }
  ...
  TStreamRec = record
{    ObjType: Word;}
    ObjType: SmallWord; { Um LoKon Win3.1 kompatibel zu bleiben. }
    VmtLink: Word;
    Load: Pointer;
    Store: Pointer;
    Next: PStreamRec;
...
{   Count: Integer;
    Limit: Integer;
    Delta: Integer;}
    Count: SmallInt; { LoKon Win31 kompatibel. }
    Limit: SmallInt;
    Delta: SmallInt;
...
{                cmp    eax,[edx].TStreamRec.ObjType}
                cmp     ax,[edx].TStreamRec.ObjType {LoKon Win31 kompatibel.}
                je      @@3
...
{               mov     ecx,[edx].TStreamRec.ObjType}
                mov     cx,[edx].TStreamRec.ObjType {LoKon Win31 kompatibel.}
              @@4:
                push    edx
{               push    ecx                    { Write object type  }
                push    cx                     { LoKon Win31 kompatibel. }
                mov     eax,esp
                push    eax                     { [1]:Pointer = Buf  }
                push    4                       { [2]:DWord   = Size }
...
function TStream.StrRead2: PChar;
var
  L : Word;
  P: PChar;
  c : char;
...
function TStream.ReadSmallWord : SmallWord;
var
  value : SmallWord;
begin
  Read(value, SizeOf(value));
...
{ S.Read(Count, SizeOf(Integer) * 3);}
  S.Read(Count, SizeOf(Count)+SizeOf(Limit)+SizeOf(Delta)); { LoKon Win31 kompatibel. }
  Items := nil;
  C := Count;
  I := Limit;
  Count := 0;
...
{               cmp     edx,[edi].TCollection.Count}
                cmp     dx,[edi].TCollection.Count { LoKon Win31 kompatibel.}
                jge     @@1
...
{                mov     ecx,[edi].TCollection.Count
                sub     ecx,edx}
                mov     cx,[edi].TCollection.Count { LoKon Win31 kompatibel. }
                sub     cx,dx
                je      @@2
                cld
                mov     edi,[edi].TCollection.Items
...
{                cmp     ecx,[edi].TCollection.Limit}
                cmp     cx,[edi].TCollection.Limit { LoKon Win31 kompatibel. }
                jne     @@1
                push    ecx
                push    edx
{                add     ecx,[edi].TCollection.Delta}
                add     cx,[edi].TCollection.Delta { LoKon Win31 kompatibel. }
...
{                mov     ecx,[edx].TCollection.Count}
                xor ecx, ecx
                mov     cx,[edx].TCollection.Count { LoKon Win31 kompatibel. }
                jecxz   @@3
                mov     ebx,Test
...
{                mov     [edx].TCollection.Count,edi}
                mov     [edx].TCollection.Count,di { LoKon Win31 kompatibel. }
              @@3:
end;
