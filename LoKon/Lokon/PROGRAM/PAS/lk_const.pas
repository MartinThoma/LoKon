...
function makelongint(A : tPoint) : longint;
function makepoint(l : longint) : tPoint;

procedure StoreInt(var t : Text; name : String; val : longint);
procedure StoreInt_(var t : Text; val : longint);
procedure StorePoint(var t : Text; name : String; val : tPoint);
...
type
  ...
  tSmallIntArray = array [0..$7ffe] of SmallInt;
...
function ReadPoint16(var S : TStream; Num : Integer) : pPointArray;
function ReadInteger16Array(var S : TStream; Num : Integer) : pIntegerArray;
...
var
  ...
  i : Integer;
begin
  GetMem(p, Num * SizeOf(tPoint));
  for i := 0 to num-1
  do begin
    p^[i].X := S.ReadSmallInt;
    p^[i].Y := S.ReadSmallInt;
  end;
  ReadPoint16 := p;
end;
...
var
  ...
  i : Integer;
begin
  GetMem(p, Num * SizeOf(Integer));
  for i := 0 to num-1
  do begin
    p^[i] := S.ReadSmallInt;
  end;
  ReadInteger16Array := p;
end;

function makelongint(A : tPoint) : longint;
begin
  makelongint := makelong(A.X, A.Y);
end;
