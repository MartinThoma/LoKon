...
begin
  state := ReadInteger16Array(S, NumIO);
  LoadInOut := true;
end;
...
function tIOSMacroItem.GetInOutPos(Num : Integer) : tPoint;
var
  A : tPoint;
  fz : integer;
begin
...
begin
  ...
  do begin
    ...
    Direction := i;
    i := Integer(GetDlgItemInt(hWindow, 105, translated, False));
    if ( i <> NumIO ) and ( i>=0 )
...
begin
  ...
  with ios
  do begin
    ios.InIdx := S.ReadSmallInt;
    ios.OutIdx := S.ReadSmallInt;
    ios.StateIdx := S.ReadSmallInt;
