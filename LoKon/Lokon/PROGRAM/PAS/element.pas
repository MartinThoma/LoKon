...
type
  ...
  tElement = object (tChapEle)
    ...
    { InOut-Funktionen. }
    function PosInOut(Num : Integer) : tPoint;
...
begin
  WinHelp(hWindow, 'LOKON.HLP', HELP_CONTEXT, 510);
end;
...
begin
  WinHelp(hMainWin, 'LOKON.HLP', HELP_CONTEXT, 525);
end;
...
begin
  WinHelp(hMainWin, 'LOKON.HLP', HELP_CONTEXT, 521);
end;
...
begin
  WinHelp(hMainWin, 'LOKON.HLP', HELP_CONTEXT, 600);
end;
...
begin
  ...
  NumState := S.ReadSmallInt;
  Input := ReadPoint16(S, NumIn);
  Output := ReadPoint16(S, NumOut);
  if NumState > 0
  then begin
    StGrFlag := S.ReadShortInt;
    if StGrFlag = 0
    then begin
  ...
  InitVal := ReadInteger16Array(S, NumIn+NumState);
end;
...
function tElement.PosInOut(Num : Integer) : tPoint;
begin
  if Num > 0
  then PosInOut := Input^[Num-1]
  else PosInOut := Output^[(-Num)-1]
end;
...
begin
  ...
  Time := ReadInteger16Array(S, NumOut);
end;
...
begin
  ...
  do begin
    ...
    GS.Seek(0);
    SetNumInOut(
      Integer(GetDlgItemInt(hWindow, id_NumIn, translated, TRUE)),
      Integer(GetDlgItemInt(hWindow, id_NumOut, translated, TRUE)),
      Integer(GetDlgItemInt(hWindow, id_NumState, translated, TRUE)) );
    NumMerke := Integer(GetDlgItemInt(hWindow, id_NumMerke, translated, TRUE));
    StrDispose(Bool);
...
begin
  WinHelp(hMainWin, 'LOKON.HLP', HELP_CONTEXT, 522);
end;
...
begin
  ...
  then begin
    ...
    S.Read(p^, i);
    { Versions„nderung abfragen. }
    version_ := S.ReadSmallInt;
    l := S.ReadSmallInt;
    SethMacroWindow(
      p,
      hMacroWin_);
