...
begin
  WinHelp(hMainWin, 'LOKON.HLP', HELP_CONTEXT, 300);
end;
...
begin
  WinHelp(hWindow, 'LOKON.HLP', HELP_CONTEXT, 300);
end;
...
begin
  BeginDrag;
  i := p^.GetInOutNr(A);
  Poi := New(pConnection, Init(i, 0, p, nil));
  (pConnection(Poi))^.setCon2Point(p^.GetInOutPos(i));
  p^.SetCon(i, Poi, 1);
...
var
  ...
  translated : Bool;
begin
  i := GetDlgItemInt(hWindow, 130, translated, False);
  if i <= 0 then i := 1;
...
var
  ...
  translated : Bool;
begin
  NumIn := Integer(GetDlgItemInt(hWindow, 100, translated, FALSE));
  NumOut := Integer(GetDlgItemInt(hWindow, 101, translated, FALSE));
  NumLines := Integer(GetDlgItemInt(hWindow, 102, translated, FALSE));
  Time := Integer(GetDlgItemInt(hWindow, 130, translated, FALSE));
...
begin
  WinHelp(hWindow, 'LOKON.HLP', HELP_CONTEXT, 200);
end;
